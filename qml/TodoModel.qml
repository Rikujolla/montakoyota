...
ListModel {
     ...
     {
         tx.executeSql('CREATE TABLE IF NOT EXISTS todo(name TEXT, done BIT, path TEXT)', []);
     }
     ...
     function addTodo(name) {
         __db().transaction(
             function(tx) {
                 __ensureTables(tx);
                 tx.executeSql("INSERT INTO todo VALUES(?,0,?)", [name,"pathi"]);
                 fillModel();
                 updateNumberOfTodos();
                 updateNumberOfDone();
             }
     ...
     function updateStatus(index, name, done, path) {
         __db().transaction(
                     function(tx) {
                         if(done) {
